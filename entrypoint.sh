...
if [ -n "$SHOPIFY_LOCALE" ]; then
    ...
    # Check task is set and back -> do Backup
        ...
        mkdir -p $THEME_PATH/settings_locale/$SHOPIFY_LOCALE
        theme download -e production-$SHOPIFY_LOCALE --config=config.yml --dir=$THEME_PATH/settings_locale/$SHOPIFY_LOCALE config/settings_data.json $INPUT_ARGS && \
        theme download -e production-$SHOPIFY_LOCALE --dir=$THEME_PATH --ignored-file=config/settings_data.json $INPUT_ARGS 
    ...
    if { [ -n "$TASK" ] && [ "$TASK" = "BACKUP" ] ;} then
        ...
        theme download -e production --dir=$THEME_PATH $INPUT_ARGS 
    elif { [ -n "$TASK" ] && [ "$TASK" = "DEPLOY" ] ;} then
